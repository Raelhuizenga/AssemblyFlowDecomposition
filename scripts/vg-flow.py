...
from flow_decomposition_old import mfd_algorithm, build_contig_flow_graph, convert_flow_path_to_contig_path
...
def main():
    ...
    graph, paths, adj_out, start_info, vg_to_contigs = build_adj_info(
                graph, paths, nvert, low_ab_nodes, args.trim)
    ...
    print(f"Discarded {len(discarded)} contigs that bridge branches.")
    transitive_edges = get_transitive_edges(adj_out)
    for (v1, v2) in set(discarded + transitive_edges):
        adj_out[v1].remove(v2)
    print(f"Discarded transitive edges {len(transitive_edges)} ")
    ...
    if args.toboggan:
        ...
        #         args.threads)
        flow_graph_new, node_dict_new = build_contig_flow_graph(adj_out, contig_abundances)
        sol, contig_paths, slack = mfd_algorithm(flow_graph_new)
        cg_paths = convert_flow_path_to_contig_path(flow_graph_new, node_dict_new, contig_paths)
    else:
...
def optimize_flow(
    ...
    # solve ILP
    print("\n*** Running optimization to induce flow on the graph***\n")
    m.optimize()
...
def write_intermediate_files(graph, paths, adj_out, start_info, contig_ab=None):
    ...
    with open("trimmed_contigs.fasta", 'w') as f:
        with open("trimmed_contigs.paths", 'w') as f2:
            for i, (c, path) in enumerate(paths.items()):
                try:
                    abundance = "{}x".format(contig_ab[c])
...
def get_adj_out(paths, g, low_ab_nodes, del_set):
    ...
    print("# conflicting edges: {}".format(len(conflict_list)))
